...
@dashboard_router.get("/{username}/stats",
    tags=["User Analytics"],
...
@dashboard_router.get("/{username}/repos",
...
async def get_user_repos_async(username: str = Path(..., description="GitHub username")):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        tasks = [fetch_repo_details(client, repo, token) for repo in repos if isinstance(repo, dict)]
        results = await asyncio.gather(*tasks)
        
        return [res for res in results if res is not None]

@dashboard_router.get("/{username}/commits",
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for repo in repos:
            if isinstance(repo, dict) and "name" in repo and isinstance(repo.get("owner"), dict) and "login" in repo["owner"]:
                ...
                tasks.append(get_all_commits_for_repo_async(client, owner, repo_name, username, token))
